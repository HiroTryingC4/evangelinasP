          ...
          {dayBookings.length === 0 ? (
            ...
            <div className="space-y-2">
              {(() => {
                ...
                return (
                  <div key={b.id} className={`card p-3 sm:p-4 flex items-start gap-3 border-l-4 ${UNIT_COLORS[b.unit] ?? "border-l-gray-300"} ${index === 1 ? "shadow-sm" : ""}`}>
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-xs sm:text-sm font-bold flex-shrink-0 ${UNIT_BADGE[b.unit] ?? "bg-gray-100 text-gray-700"}`}>
                      {b.unit}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        <span className="font-semibold text-gray-900 text-sm sm:text-base">{b.guestName}</span>
                        <div className="flex gap-1 flex-wrap">
                          {isIn && <span className="text-xs px-2 py-0.5 bg-green-100 text-green-800 rounded-full font-medium">Check-in {b.checkInTime}</span>}
                          {isOut && <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-800 rounded-full font-medium">Check-out {b.checkOutTime}</span>}
                        </div>
                      </div>
                      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500">
                        {b.contactNo && <span className="flex items-center gap-1"><Phone className="w-3 h-3" />{b.contactNo}</span>}
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{formatDate(b.checkIn)} → {formatDate(b.checkOut)}</span>
                        <span className="font-medium text-gray-700">{formatPHP(b.totalFee)}</span>
                      </div>
                      <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLOR[b.paymentStatus] ?? ""}`}>
                          {b.paymentStatus}
                        </span>
                        {b.remainingBalance > 0 && (
                          <span className="text-xs text-red-600 font-semibold flex items-center gap-1">
                            <AlertCircle className="w-3 h-3" />{formatPHP(b.remainingBalance)} due
                          </span>
                        )}
                        {b.remainingBalance === 0 && (
